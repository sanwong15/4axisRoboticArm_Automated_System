...
def evaluate( net, test_loader ):
    ...
    for it, batch in enumerate(test_loader, 0):
        #images = batch['tif'][:,1:,:,:] #IR R G B to R G B
        images = batch['tif'] #IR R G B to R G B
...
def do_training(out_dir='../../output/resnet34_out'):
    ...
    train_dataset = KgForestDataset('train_35479.txt',  #'train-32479',#'train-32479-unsuper-31936' , #'train-3000',#'train-8000',#'debug-32', #'train-3000',#'train-8000',###
                                    transform=[
                                        tif_color_corr,
                                        augment,
                                        img_to_tensor,
    ...
    test_dataset = KgForestDataset('val_5000.txt', #'valid-8000', ##'debug-32', ###'train-40479',  ##'train-ordered-20', ##
                                height=SIZE, width=SIZE,
                                transform=[
                                    tif_color_corr,
                                    img_to_tensor,
    ...
    for epoch in range(start_epoch, num_epoches):  # loop over the dataset multiple times
        ...
        for it, batch in enumerate(train_loader, 0):
            #images = batch['tif'][:,1:,:,:] #IR R G B to R G B
            images = batch['tif'] #IR R G B to R G B
            #images = torch.ones(20,3,256,256) #IR R G B to R G B
            labels = batch['label'].float()
