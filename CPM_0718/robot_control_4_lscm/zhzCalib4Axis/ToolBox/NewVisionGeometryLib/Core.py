...
class VGLError(Exception):
    ...
    Examples
    --------
    >>> import VisionGeometryLib as VGL
    >>> VGL.T2Rt(None)
...
def __checkSize(array, size, msg=None):
    if array.size != size:
        if msg is None:
            raise VGLError('%d size array given. Array must be %d size' % (array.size, size))
        else:
            raise VGLError(msg)
...
def __checkShape(array, shape, msg=None):
    if array.shape != shape:
        if msg is None:
            raise VGLError('%s shape array given. Array must be %s shape' % (str(array.shape), str(shape)))
        else:
            raise VGLError(msg)
...
def __checkNdim(array, ndim, msg=None):
    if array.ndim != ndim:
        if msg is None:
            raise VGLError('%d-dimensional array given. Array must be %d-dimensional' % (array.ndim, ndim))
        else:
            raise VGLError(msg)
...
def isRotationMatrix(R_3x3):
    ...
    R = __toArray(R_3x3)
    __checkShape(R, (3, 3))
    ShouldBeNormOne = np.allclose(np.linalg.norm(R, axis=0), np.ones(shape=(3)), atol=1.e-3)
    ShouldBePerpendicular = \
        np.allclose(np.cross(R[:, 0], R[:, 1]), R[:, 2], atol=1.e-3) \
        and np.allclose(np.cross(R[:, 1], R[:, 2]), R[:, 0], atol=1.e-3) \
        and np.allclose(np.cross(R[:, 2], R[:, 0]), R[:, 1], atol=1.e-3)
...
def rt2T(rVec, tVec):
    ...
    >>> import numpy as np
    >>> import VisionGeometryLib as VGL
    >>> rVec = np.array([0.78539816, 0, 0])   # shape can be (3, 1), (1, 3), (3,) or any array_like which has 3 elements
    >>> tVec = np.array([7, 8, 9])            # shape can be (3, 1), (1, 3), (3,) or any array_like which has 3 elements
    >>> VGL.rt2T(rVec, tVec)
    array([[ 1.        ,  0.        ,  0.        ,  7.        ],
...
def computeRotateVec(vec0, vec1, rotateAxis=None):
    ...
    rotateAxis : array_like, optional
        Default is None, when vec0 and vec1 in opposite direction(-180 deg), use given rotate axis,3x1 or 1x3 or 3.
...
def reconstruct3DPts(imgPtsA_2xn, imgPtsB_2xn, cameraMatrixA, cameraMatrixB,
    ...
    if calcReprojErr:
        ...
        ProjImgPtsA_2xn = projectPtsToImg(PtsInCoordinateX_3xn_Norm, Tx2CamA_4x4, CameraMatrixA, DistCoeffsA)
        RpErrA = np.linalg.norm(UnDistortPtsA_2xn - ProjImgPtsA_2xn, axis=0).mean()

        ProjImgPtsB_2xn = projectPtsToImg(PtsInCoordinateX_3xn_Norm, Tx2CamB_4x4, CameraMatrixB, DistCoeffsB)
